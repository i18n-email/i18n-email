---
title: Introduction
description: Translate transactional emails into any language using AI.
icon: Globe
---

**i18n-email** translates transactional emails into any language using AI. Pass a React Email component, a raw HTML string, or plain text, specify a target locale, and get back a translated `subject`, `html`, and `text` — ready to send.

## Features

- Works with **React Email components**, **raw HTML strings**, or **plain text**
...
When `text` is passed, the HTML pipeline is skipped entirely. Only `[subject, text]` are sent to OpenAI. The result is `{ subject, html: undefined, text }`.
